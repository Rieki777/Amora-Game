...
async function startServer() {
   ...
   * hard lesson is that a cache only ever written by the code that also
   * writes its source is a number nobody checks. Re-derive it at boot and say
   * so out loud when it had drifted. Off costs nothing: a village that never
   * turned messaging on has no rows to walk.
   */
  if (effectiveLifecycle("messaging") !== "off") {
    const cache = await auditLastMessageAt(getPool());
    if (cache.drifted.length) {
      console.error(
        `[messaging] last_message_at drifted on ${cache.drifted.length} of ${cache.checked} conversation(s), repaired: ${cache.drifted.slice(0, 10).join(", ")}`,
      );
    }
  }

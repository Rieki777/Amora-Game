...
export async function sendGratitude(deps: GratitudeDeps, input: SendInput): Promise<SendOutcome> {
  ...
  const result = await writeGratitudeRow(
     ...
     * nothing around it. `postTransfer` rolls back and RETHROWS on any database
     * error — a lock wait, a dropped connection — so a throw there left the
     * note committed, the cycle's allowance spent, and nothing in the
     * recipient's hands: a record saying gratitude was given, and no gratitude.
     * A retry does not heal it either, because a retry writes a NEW note id and
     * is therefore a second charge. Exactly the shape `give()` had on the other
     * door, and exactly the shape `server/lib/voiceClaim.ts` catches on a third.
     *
  ...
  const toBalance = result.credit?.toBalance;
  if (toBalance !== undefined) {
    await deps.members.update(recipient.id, (u: any) => {
      u.recognitionBalance = toBalance;
    });
  }

...
# …and the groups have to actually move into it. Leaving them in `#icons`
# while restyling `#badges .bgroup` was invisible rather than obviously
# broken: `.bseal` is absolute and `bg.style.transform` makes any element a
# containing block, so the marks still landed in the right places, still
# under every building, with the zoom-gate fade quietly dead.
swap(
    """  const g=document.createElement('div');g.className='bgroup';
  iWrap.appendChild(g);bgEls[s.key]=g;return g;""",
    """  const g=document.createElement('div');g.className='bgroup';
  ($('badges')||iWrap).appendChild(g);bgEls[s.key]=g;return g;""",
)
swap(
    "const bgEls={}; // one group per structure, a sibling of the poi inside #icons",
    "const bgEls={}; // one group per structure, in the badge plane above the labels",
)
...
# ── 5. Placement: record here, resolve for the whole land afterwards ─────
# A ring keeps one building's marks apart. It says nothing about the
# building next door, and the Village Heart is exactly where buildings stand
# next door to each other. So this block only records; layoutBadges below
# settles the whole land at once.
swap(
    """        if(show){
...
# ── 5b. One settlement for the whole land ────────────────────────────────
rep(
...
rep(
    ...
    r"""/* Where every mark ends up. Runs once per frame for the whole land, after
   the buildings have moved, because two neighbours can each hold their own
   marks apart and still collide with each other. That pair is what the
   testing screenshots showed, and no per-building rule can see it. */
function layoutBadges(){
  const groups=[];
  for(const s of SCENE.structures){
    const bg=bgEls[s.key];if(!bg||!bg._on)continue;
    const kinds=(bg.dataset.kinds||'').split(',').filter(Boolean);
    const off=bg._off||30;
  ...
  for(const g of groups){
    ...
    if(g.cluster){
      const more=g.bg.querySelector('.b-more');
      if(more){more.style.left=(-0.82*CR(g))+'px';more.style.top=(0.57*CR(g))+'px'}

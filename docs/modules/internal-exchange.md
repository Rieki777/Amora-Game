# Module design: Internal Exchange — Wallet + Village Exchange: buy and swap platform-governed closed-loop credits, treasury as counterparty, everything settling through the one token ledger
...
**A Uniswap-looking, nothing-like-Uniswap village shop: members buy internal credits with fiat (Stripe) or via admin-recorded cash, and swap internal credits against the village treasury at admin-governed posted prices — while Hypha-governed tokens (the village's equity token, Voice, project shares) appear in the same wallet strictly read-only with a "Trade on Hypha" deep link.**

Estimated sessions: 7

## Design decisions, and why

- Resolved a contradiction in the original concept, which showed buying 'Restaurant Tokens' (ownership of a village asset) in the internal exchange while also holding the rule that tradeable, share-like tokens are issued on Hypha. Design: the internal exchange executes trades ONLY on platform-governed closed-loop credits; anything share-like (an equity token, Voice, project or business tokens) renders in the same wallet UI as a read-only balance card with a 'Trade on Hypha' deep link to the village's configured DHO. That is the default for every deployment.
...
- Cash-economy reality: in most villages a good share of money moves as cash, a bank transfer, or a local rail like SINPE in Costa Rica, so manual admin credit is a first-class v1 order kind with a reference field and a receipt rather than an afterthought. The original concept assumed everyone had a crypto wallet.
...
- exchange.anchor_currency: 'USD' (any ISO 4217 code Stripe presents in) — unit of account all credit prices are posted in; also the Stripe presentment currency
...
- Expectation gap: the original concept described 'shares of the different village projects' swapping here. This design deliberately refuses that, because in-platform share swaps are a securities-exchange posture and a fundamentally different product. A village asking for them is not asking for a setting, and should be told so plainly rather than accommodated quietly.
...
- Scope creep magnet: accommodation auto-charge and library credits mechanics will want to live in this module. They are separate modules that SPEND credits through the same ledger; the exchange only sells and swaps them. Keep the boundary or this becomes the god-module.
...
- Where does the stay-credit SPEND side live? Accommodation auto-charge is a separate module that debits the same ledger; the exchange only sells credits. Confirm that module boundary before anyone builds nightly billing into the exchange.
...
- Receipt numbering per calendar year (PREFIX-2026-000123, resets annually, config-driven prefix) or a single monotonic sequence forever?

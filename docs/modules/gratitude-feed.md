...
6. creditTokens(ledger, {userId: author, amount, source:'gratitude_received', sourceRef: entry.id, idempotencyKey: keys.gratitudeGiven(villageId(), entry.id)}) — identical source/key scheme as the wall send; recipient's recognitionBalance set to the RECOMPUTED balance (never +=).

...
- (existing, referenced not redefined) gratitude.base_budget, gratitude.max_per_recipient_per_cycle, gratitude.cycle_mode — the send-budget side; and the existing tokens.equity_address / tokens.voice_address / tokens.base_rpc_url variables become the seed values for the two hypha registry rows

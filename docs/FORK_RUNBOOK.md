...
- `tokens` table rows (0006 seeds gratitude/amora/voice) — a fork renames its
  recognition token HERE AND NOWHERE ELSE, through Admin → Tokens. The
  registry is the single source of truth for a token's display name
  (`mergedConfig()` reads `tokens`.`name` ahead of both the brand overlay and
  `shared/gameConfig.ts`), and the slug is the primary key and never changes.

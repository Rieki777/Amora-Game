...
> **Dated 2026-08-29. This is the 2026-08-08 design record, and the build took a
> different road from section 5 onward.** It sat uncommitted in the primary
> checkout for three weeks and is committed here so the eleven decisions locked
> with Rye survive somewhere readable. Read it as the record of what was
> decided, and check the code before treating any of it as a build plan.
>
> **What still stands.** Sections 0 to 4: the three altitudes, the mirror
> principle, the decision register D1 to D11, the Age of Empires and
...
> exists; PixiJS is not a dependency. What shipped instead is a self-contained
> HTML artifact served at `/grounds/index.html` inside an iframe. Section 5.2's
> six normalised tables (`map_scenes`, `map_zones`, `map_structures`,
> `map_flows`, `map_structure_facts`, `map_edits`) were never created. The
> scene is stored as a document, in `map_scene_drafts` and
> `map_scene_revisions`. Section 6's phase gates name a game variable
> (`map.grounds_enabled`), a route (`/api/map/pulse`) and a launch requirement
> (`grounds:scene-published`) that no commit has ever added, so G0 through G5
> are not a ladder anyone is standing on.
>
> D7 and D8 were filed as proposed and never accepted. Treat them as declined.

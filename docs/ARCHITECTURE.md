...
12. **Routes serve** (2130 onward). Express app, webhook seam, middleware,
    539 route registrations in `server/index.ts` plus the `register(app,
...
4. **Routes** in `server/routes/<id>.ts`, **not** in `server/index.ts`,
   ...
   **Why not `server/index.ts` any more.** That file is 32,864 lines with
   539 route registrations in one `startServer` closure, and this recipe
